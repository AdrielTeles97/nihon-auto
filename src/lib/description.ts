...
function wpOrigin(): string {
    const pub = (
        process.env.NEXT_PUBLIC_WP_SITE_ORIGIN ||
    ...
    if (pub) {
        try {
            const u = new URL(
                pub.includes('/wp-json/') ? pub : pub + '/wp-json/'
            )
            return u.origin
        } catch {}
    }
    // Fallback padrão (igual ao usado no serviço server-side)
    return 'https://https://nihonacessoriosautomotivos.com.br/'
}
...
function absolutizeRelativeUrls(input: string): string {
    ...
    out = out.replace(
        /\sdata-(?:lazy-)?(?:src|original)=(['"])(https?:[^'"\s>]+|\/\/[^'"\s>]+|\/(?!\/)[^'"\s>]+)\1/gi,
        (_m, q, val: string) => ` src=${q}${val}${q}`
    )
    out = out.replace(
        /\sdata-srcset=(["'])([^"']+)\1/gi,
        (_m, q, val: string) => ` srcset=${q}${val}${q}`
    )
    ...
    out = out.replace(
        /\ssrc=(["'])(?!https?:|\/\/|data:|#|\/)\s*([^"'>\s]+)\1/gi,
        (_m, q, p: string) =>
            ` src=${q}${origin}/${p.replace(/^\.?\//, '')}${q}`
    )
    out = out.replace(
        /\shref=(["'])(?!https?:|\/\/|#|\/)\s*([^"'>\s]+)\1/gi,
        (_m, q, p: string) =>
            ` href=${q}${origin}/${p.replace(/^\.?\//, '')}${q}`
    )
