...
export async function GET(request: NextRequest) {
    try {
        ...
        // Converter slugs de marcas em IDs
        const allBrandIds: number[] = [...brandIds]
        if (brandSlugs.length) {
            try {
                console.log('Converting brand slugs:', brandSlugs)
                const brandsResp = await wpApi.get('/products/brands', {
                    params: { per_page: 100 }
                })
                const brandMap = new Map(
                    brandsResp.data.map((b: { slug: string; id: number }) => [
                        b.slug,
                        b.id
                    ])
                )
                console.log('Available brands:', Array.from(brandMap.entries()))
                for (const slug of brandSlugs) {
                    const brandId = brandMap.get(slug)
                    console.log(
                        `Looking for slug '${slug}', found ID:`,
                        brandId
                    )
                ...
                console.error('Erro ao converter slugs de marcas:', error)
            }
        }

        console.log('Final brand IDs for API:', allBrandIds)

        if (allBrandIds.length) params.brand = allBrandIds.join(',')
