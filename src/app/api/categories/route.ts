...
export async function GET(request: NextRequest) {
  try {
    ...
    const responseData = await cached(
      ['wc:categories', page, perPage, parent ?? '', search.toLowerCase()],
      async () => {
        const resp = await wpApi.get<WCCategory[]>('/products/categories', {
          params: {
    ...
    return new NextResponse(
      JSON.stringify({ success: true, data: categories, total: categories.length }),
