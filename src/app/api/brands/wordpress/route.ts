...
export async function GET(request: NextRequest) {
    try {
        console.log('🚀 API /api/brands/wordpress chamada')
        ...
        if (logocloud) {
            console.log('🎨 Buscando marcas para LogoCloud...')
            const brands = await brandsService.getBrandsForLogoCloud(limit)
            console.log('📊 Marcas retornadas:', brands.length)
...
export async function PUT(request: NextRequest) {
    try {
        ...
        if (action === 'sync') {
            ...
            const wpBrands = await brandsService.getWordPressBrands()
            const brands =
                brandsService.convertWordPressBrandsToBrands(wpBrands)
