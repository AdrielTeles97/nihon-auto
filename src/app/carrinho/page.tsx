...
export default function CarrinhoPage() {
  ...
  const handleEmailQuote = () => {
    const subject = 'Solicitação de Orçamento - Montecarlo';
    const body = `Olá!\n\nGostaria de solicitar um orçamento para os seguintes produtos:\n\n${cart.items.map(item => 
      `• ${item.product.name} (Qtd: ${item.quantity}) - R$ ${(item.product.price * item.quantity).toFixed(2).replace('.', ',')}`
    ).join('\n')}\n\nTotal: R$ ${total.toFixed(2).replace('.', ',')}\n\nAguardo retorno.\n\nObrigado!`;
    
    const mailtoUrl = `mailto:contato@montecarlo.com.br?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <div className="flex justify-between">
                  <span>Subtotal ({cart.items.reduce((acc, item) => acc + item.quantity, 0)} itens):</span>
            ...
            {/* Actions */}
            <div className="space-y-3">
              <Button onClick={handleWhatsAppQuote} className="w-full bg-green-600 hover:bg-green-700">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <h4 className="font-semibold text-sm mb-2">Compra Segura</h4>
