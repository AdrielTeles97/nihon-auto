    ...
    <footer className="bg-primary text-primary-foreground">
      <div className="container mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <span className="font-playfair text-xl font-bold block">
                  {loading ? 'Carregando...' : companyData.company_name}
                </span>
            ...
            <p className="text-sm text-primary-foreground/80 text-pretty">
              {loading ? 'Carregando informações...' : companyData.company_description}
            </p>
            <div className="flex space-x-3">
              {companyData.social_media.facebook && (
                <Button 
                  ...
                  asChild
                >
                  <a href={companyData.social_media.facebook} target="_blank" rel="noopener noreferrer">
              ...
              {companyData.social_media.instagram && (
                <Button 
                  ...
                  asChild
                >
                  <a href={companyData.social_media.instagram} target="_blank" rel="noopener noreferrer">
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-2 text-sm text-primary-foreground/80">
              <li>
                <Link href="/produtos" className="hover:text-primary-foreground transition-colors">
                  Todos os Produtos
                </Link>
              </li>
              <li>
                <Link href="/a-nihon" className="hover:text-primary-foreground transition-colors">
                  Sobre a Nihon
                </Link>
              </li>
              <li>
                <Link href="/carrinho" className="hover:text-primary-foreground transition-colors">
                  Carrinho de Compras
                </Link>
              </li>
              <li>
                <a href={`https://wa.me/${companyData.whatsapp}`} target="_blank" rel="noopener noreferrer" className="hover:text-primary-foreground transition-colors">
                  Solicitar Orçamento
                </a>
              </li>
              <li>
                <a href={`mailto:${companyData.email}`} className="hover:text-primary-foreground transition-colors">
                  Entre em Contato
                </a>
              </li>
            </ul>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3 text-sm text-primary-foreground/80">
              <div className="flex items-center space-x-2">
                ...
                >
                  {loading ? 'Carregando...' : formatPhone(companyData.whatsapp)}
                </a>
              </div>
              <div className="flex items-center space-x-2">
                ...
                <a 
                  ...
                  className="hover:text-primary-foreground transition-colors"
                >
                  {loading ? 'Carregando...' : companyData.email}
                </a>
              </div>
              <div className="flex items-center space-x-2">
                <Clock className="h-4 w-4" />
                <span>{loading ? 'Carregando...' : companyData.working_hours}</span>
              </div>
              <div className="flex items-start space-x-2">
                <MapPin className="h-4 w-4 mt-0.5" />
                <span>{loading ? 'Carregando endereço...' : companyData.address}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-primary-foreground/20 mt-12 pt-8 text-center text-sm text-primary-foreground/60 space-y-1">
          <p>&copy; {year} {loading ? 'Nihon Peças e Acessórios' : companyData.company_name}. Todos os direitos reservados.</p>
          <p>
