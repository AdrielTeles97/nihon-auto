  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!url.searchParams.get('per_page')) url.searchParams.set('per_page', '12')
    ...
    <div className="flex flex-col lg:flex-row gap-6">
      ...
      <div className="lg:w-64 flex-shrink-0">
        <div className="bg-white border rounded-lg p-4">
          <h3 className="font-semibold text-gray-800 mb-4 pb-2 border-b">Filtrar produtos</h3>
          ...
          <div className="mb-6">
            ...
            <div className="space-y-2">
              {(() => {
                ...
                const allCount = (categories || []).reduce((acc, c) => acc + (c.count ?? 0), 0)
                const cleaned = (categories || []).filter(c => c.slug !== 'sem-categoria' && c.slug !== 'uncategorized')
                const listed = [{ id: 0, slug: '__all__', name: 'Todas', count: allCount, description: '', image: null, parentId: null }, ...cleaned]
                const slice = listed.slice(0, showAllTypes ? listed.length : 5)
                  ...
                  <div key={cat.slug} className="flex items-center justify-between text-sm">
                    <label className="flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        className="mr-2"
                        checked={cat.slug === '__all__' ? selectedCategories.length === 0 : selectedCategories.includes(cat.slug)}
                        onChange={() => (cat.slug === '__all__' ? updateQuery('category', []) : handleCategoryCheck(cat.slug))}
                      />
                      <span className="text-blue-600 hover:underline">{cat.slug === '__all__' ? 'Todas' : cat.name}</span>
                    </label>
                    <span className="text-gray-500">({cat.count ?? 0})</span>
                  </div>
                ))
              })()}
              {!showAllTypes && categories.length > 5 && (
                <button onClick={() => setShowAllTypes(true)} className="text-blue-600 text-sm hover:underline">
                  Ver mais...
                </button>
              )}
            </div>
          ...
          <div className="mb-6">
            ...
            <div className="space-y-2">
              {(brands || [])
                .slice(0, showAllManufacturers ? brands.length : 5)
                .map((b) => (
                  <div key={b.slug} className="flex items-center justify-between text-sm">
                    <label className="flex items-center cursor-pointer">
                      ...
                      />
                      <span className="text-blue-600 hover:underline">{b.name}</span>
                    </label>
                    <span className="text-gray-500">({b.count ?? 0})</span>
                  </div>
                ))}
              {!showAllManufacturers && brands.length > 5 && (
                <button onClick={() => setShowAllManufacturers(true)} className="text-blue-600 text-sm hover:underline">
                  Ver mais...
                </button>
              )}
            </div>
